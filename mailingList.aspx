<%@ Page Language="C#" AutoEventWireup="true" CodeFile="~/mailingList.aspx.cs" Inherits="mailingList" %>
  ...
  <link href="js/themes/base/jquery.ui.dialog.css" rel="stylesheet" type="text/css" />
    ...
    <link href="js/themes/base/jquery.ui.core.css" rel="stylesheet" type="text/css" />
		...
		<style type="text/css">
			...
			.menu-mailinglist {
             background-image: url('images/icon-menu/contacts0.png');
			}
...
//    });
   ...
   $("#frm").css("padding", "0px");
            $("#frm").dialog(
            {
                ...
                title: "<span style='padding:200px;font: bold 18px Arial;'><%=Resources.Resource.create_audio_message%></span>",
          ...
          //$('#frmStepty').attr('src', "step.aspx");
            $("#BaddProject").click(function () {
                var UserActive = '<%= useractive %>';
                if (UserActive == 'False') {
                    $("span.ui-dialog-title")._hide();
                    $('#frmStepty').attr('src', "NotActive.aspx");
                    $('#frm').dialog("open");
                }

                else {
                    if (document.getElementById("frmStepty").src.indexOf("step.aspx") == -1)
                        $('#frmStepty').attr('src', "step.aspx");
                    $("#frm").dialog('option', 'title', "<span style='margin-left: 220px;font-size: 20px;'><%=Resources.Resource.create_audio_message%></span>");
                    $("#frm").dialog("open");
...
//                  }
    ...
    var strFromDate = $('#timepickerfrom').val();
		...
		<script type="text/javascript">
		    ...
		    function deleteCategory(deleteFromDBcategory) {
		        var ddllist = window.document.getElementById("DDLmailingList");
		        var idCategory = ddllist.options[ddllist.selectedIndex].value;
		        var idCategoryText = ddllist.options[ddllist.selectedIndex].text;
		        $.ajax({ type: "GET", url: "streamFile.aspx?deleteCategory=" + 1 + "&idCategory=" + idCategory, crossDomain: true, success: function (result) {
		            if (result == "true" || result == "True") {
		                {
		                    retStatus = true;
		                    alert("הרשימה " + idCategoryText + " נמחקה בהצלחה");
		                    location.reload();
		                }
		            }
		    ...
		    function opendivDeleteCategory(idcategory) {
		        var ddllist = window.document.getElementById("DDLmailingList");
		        var idCategoryText = ddllist.options[ddllist.selectedIndex].text;
		        $("#mycategory").text(idCategoryText);
		        $("#divDeleteCategory").dialog('open');
//		        document.getElementById("HFidCategory").value = idcategory;
		        var idCategory = ddllist.options[ddllist.selectedIndex].value;
		        var idCategoryText = ddllist.options[ddllist.selectedIndex].text;
		        //alert(idCategoryText);
		        $(".ui-dialog-buttonpane button:contains('מחק')").css({ "width": "150px", "fontWeight": "Bold", "float": "right" });
		        $(".ui-dialog-buttonpane button:contains('מחק')").css({ "width": "150px", "fontWeight": "Bold", "float": "right" });
		    }
...
<div id="dialogNew" title="<%=Resources.Resource.creat_list_of_contacts%>">
		...
		<div id="default2" style="width:720px;margin-right: 311px;" >
                 <asp:Panel ID="Panel2" runat="server">
                    <table id="tablemenu" align="<%=Resources.Resource.align%>">
                        <tr>
                                <td id="Td1" class="menu" runat="server" >
			                    <input  type="button" id="BaddProject" class="icom-menu menu-createMsg"  />
                                <br />
                                <label  style="padding-right:5px"><%=Resources.Resource.create_audio_message%></label>
                            </td>
                                 <td class="icom-menu menu-my-account" id="tdmyaccount" runat="server" onclick="location.href='dashboard.aspx'">
                        <%--<asp:Button ID="B_myaccount"  runat="server" CssClass="icom-menu menu-my-account"
                            onclick="Button2_Click" />--%>
                           
                            <label ><%=Resources.Resource.my_account%></label>
                            </td>
                                 <td  class="icom-menu menu-project" id="tdProject" runat="server" onclick="location.href='selectProject.aspx'">
                                   <%--  <asp:Button ID="t1" runat="server" CssClass="icom-menu menu-project"
                                        onclick="Button3_Click" />--%>
                              <br />
                                <label  style="padding-right:0"><%=Resources.Resource.project_management%></label>
                            </td>
                                <td  class="icom-menu menu-mailinglist" id="tdMailingList" runat="server" onclick="location.href='mailingList.aspx'">
                         <%--<asp:Button ID="Button5" runat="server" CssClass="icom-menu menu-mailinglist"
                            onclick="Button5_Click" />--%>
                              <br />
                            <label ><%=Resources.Resource.contacts%></label>
                       </td>
                                <td class="icom-menu menu-credit" id="tdCredit" runat="server" onclick="location.href='movements.aspx'">
                        <%--<asp:Button ID="Button6" runat="server" CssClass="icom-menu menu-credit"
                            onclick="Button6_Click" />--%>
                              <br />
                            <label ><%=Resources.Resource.account_status%></label>
                        </td>
                                 <td class="icom-menu menu-update" id="tdUpdate" runat="server" onclick="location.href='updateDetailswithverify.aspx'">
                      <%--   <asp:Button ID="Button1" runat="server" CssClass="icom-menu menu-update"
                            onclick="Button1_Click" />--%>
                              <br />
                            <label ><%=Resources.Resource.update_details%></label>
                            </td>
                        </tr>
                    </table>
				...
				<fieldset title="<%=Resources.Resource.stage_3%>" style="border: oldlace;">
								...
								<table class="widthTableStep" style="direction:<%=Resources.Resource.dir%>" >
                                    <tr style="background-image:url(images/step/table-top.jpg);height:60px">
                                        <td colspan="2" style="padding-right: 10px;"><%--
                                            ...
                                            <input type="button" class="step-buttton" value="מחק רשימה זו" onclick="opendivDeleteCategory()" />
...
color: #6e6e6e;"><%=Resources.Resource.choose_all%></span>--%>
									...
									<tr>
										...
										<td width="40%" colspan="2" align="center" >
											<div align="center"  <%--style="overflow:auto;height:190px"--%> >
                                             <asp:GridView      Width="100%" ID="GridView1" runat="server" Font-Italic="False"   PageSize="200"  
                                                    AutoGenerateColumns="False"  CssClass="gridview"    AllowPaging="true"  
                                                CellPadding="4"  GridLines="Horizontal" OnRowDataBound="GridView1_RowDataBound"   onpageindexchanging="GridViewProjects_PageIndexChanging"  > 
                                                         <Columns>
                                        ...
                                        <PagerStyle CssClass="pagging-gv"/>
                                                 <RowStyle CssClass="gridviewLine" Height="25px"/>
                                                     ...
                                                     <asp:DropDownList runat="server" ID="ddlPageSize" AutoPostBack="True"  onselectedindexchanged="ddlPageSize_SelectedIndexChanged"></asp:DropDownList>
                     ...
                     <div id="divMaillistDefualt" class="divstep" style="text-align:<%=Resources.Resource.align%>" >       
                        ...
                        <div id="divDeleteCategory" style="text-align:center">
                            <label class="txt-dialog-ml"> האם אתה בטוח שברצונך למחוק את הרשימה</label>
                            <label class="txt-dialog-ml" id="mycategory"></label>
                             <label class="txt-dialog-ml" id="Label3">?</label>
                        </div>
...
<div id="dialog-confirm" >
  <p><label class="txt-dialog-ml" ><%=Resources.Resource.end_time_cannot_greater%>
      ...
      <div id="frm" style="overflow:hidden;"  >

  <!--<iframe id="frmStepty" src=""  height="530px" width="860px" frameborder="0"></iframe>-->
 <iframe id="frmStepty" src=""  height="500px" width="730px" frameborder="0" ></iframe>
   </div> 
